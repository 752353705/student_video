<template>
	<view>
		<view class="my box-boder">
			<view class="height" style="background-image: linear-gradient(to bottom, #192936, #786f68);height: 526rpx;">
				<!-- 头部 -->
				<view class="head pa-r40 pa-b20 pa-l40  box-boder">
					<view class="userImg ">
						<image v-if="userInfo.avatarUrl" class="w-100 h-100" :src="userInfo.avatarUrl" mode=""></image>
						<image @click="jump" v-else class="w-100 h-100" src="/static/avatarUrl.png" mode=""></image>
					</view>
					<view class="right ma-l10">
						<view class="right_uname item-end ma-l20">
							<view v-if="userInfo.userName" class="user_name ma-r20">{{ userInfo.userName || '昵称' }}</view>
							<view v-else class="user_name ma-r20" @click="jump">昵称</view>
						</view>
						<view class="box">
							<view class="code">{{ user_phone.slice(0, 3) }}****{{ user_phone.slice(7) }}</view>
							<view class="location">
								<view class="t-icon" :class="userInfo.gender == 2 ? 'iconbianzu' : 'iconziyuan'"></view>
				...
				<!-- 用户自己的简介 -->
				<view class="desc box-boder pa-l40">{{ userInfo.personalProfile || '' }}</view>
				<!-- 关注收藏 -->
			...
			<view :style="{ height: swiper_height }" style="border-radius:30rpx 30rpx 0 0;overflow: hidden;box-sizing: border-box;padding: 0px 5px;background-color: #f5f5f5;">
				<!-- 分类展示头 -->
				<swiperTabHead class="height" :flex="false" :tabBars="tabBars" :tabIndex="tabIndex" @tabtap="tabtap"></swiperTabHead>
				<mescroll-item
					ref="mescroll"
					:waterFullHeight="swiper_height"
					:i="i"
					:index="tabIndex"
					:tabs="tabBars"
					...
					:kw="kw"
					:listData="listData"
					v-on:getData="getMyTxt"
				></mescroll-item>
			</view>
		</view>
		<!-- 用于进行用户操作的弹出框 -->
		<uni-popup class="pop" animation="false" ref="popup_useoperation" type="operation" mask-click="false">
			<uni-popup-useoperation
				:opertop="opertop"
				:txtid="txtid"
				:operleft="operleft"
				type="success"
