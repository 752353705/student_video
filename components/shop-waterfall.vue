<template>
	<view class="list-container">
		<view id="wf-list" class="list" v-for="(list, listIndex) of viewList" :key="listIndex">
			...
			<navigator :url="'/pages/shopdetail/shopdetail?id=' + item.id" hover-class="none" >
					<!-- 商品图片 -->
					<u-lazy-load @load="handleViewRender(listIndex, index)" @error="handleViewRender(listIndex, index)" :image="item.picUrl"></u-lazy-load>
					<!-- 商品的简介 -->
					<view class="shop">
						<!-- 商品简介 -->
						<view class="shop_detail">{{ item.name }}</view>
						<!-- 商品中底部的价格，限时购 -->
